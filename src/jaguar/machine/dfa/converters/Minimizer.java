...
public class Minimizer{
    ...
    public void doConvertion(int debug_level){
        ...
        HashSet<StateSet> particiones = getParticiones(tmp);
    ...
    protected HashSet<StateSet> getParticiones(){
        return getParticiones(getDfaOrig());
    }
    /**
     * Obtiene las particiones de equivalencia
     */
    protected HashSet<StateSet> getParticiones(DFA dfa){
        HashSet<StateSet> nuevoP = ceroDistinguibles(dfa);
        HashSet<StateSet> viejoP = new HashSet<StateSet>(0);
        HashSet<StateSet> aux, tmp;
        while(!nuevoP.equals(viejoP)){
            viejoP = (HashSet<StateSet>) nuevoP.clone();
            aux = new HashSet<StateSet>();
            for(StateSet clase : nuevoP){
                tmp = refinaClass(dfa,clase,viejoP);
                aux.addAll(tmp);
            }
            nuevoP = aux;
        }
    ...
    private HashSet<StateSet> refinaClass(DFA dfa, StateSet currentClass, HashSet<StateSet> previousClasses){
      ...
      */
    protected boolean distinguibles(DFA dfa, State p ,State q, HashSet<StateSet> previousClasses){
        DfaDelta d = (DfaDelta)dfa.getDelta();
        Alphabet A = dfa.getSigma();
        Object aArray[] = A.toArray();
        State rp, rq;
        for (Symbol si : A.getSigma()) {
      ...
      *
      */
    protected HashSet<StateSet> ceroDistinguibles(DFA dfa){
     ...
     */
    protected StateSet getClaseDeEquivalencia(State p, HashSet<StateSet> particionDeEquivalencia){
